...
// ===== Default Words =====
const DEFAULT_WORDS = [
  { word: '안녕하세요', reading: 'annyeonghaseyo', japanese: 'こんにちは', example: '안녕하세요! 처음 뵙겠습니다.', notes: '挨拶', mastered: false },
  { word: '감사합니다', reading: 'gamsahamnida', japanese: 'ありがとうございます', example: '도와주셔서 감사합니다.', notes: '挨拶', mastered: false },
  { word: '죄송합니다', reading: 'joesonghabnida', japanese: 'すみません', example: '늦어서 죄송합니다.', notes: '挨拶', mastered: false },
  { word: '네', reading: 'ne', japanese: 'はい', example: '네, 알겠습니다.', notes: '基本', mastered: false },
  { word: '아니요', reading: 'aniyo', japanese: 'いいえ', example: '아니요, 괜찮아요.', notes: '基本', mastered: false },
  { word: '나', reading: 'na', japanese: '私', example: '나는 학생이에요.', notes: '代名詞', mastered: false },
  { word: '먹다', reading: 'meokda', japanese: '食べる', example: '밥을 먹다.', notes: '動詞', mastered: false },
  { word: '가다', reading: 'gada', japanese: '行く', example: '학교에 가다.', notes: '動詞', mastered: false },
  { word: '좋다', reading: 'jota', japanese: '良い・好きだ', example: '날씨가 좋아요.', notes: '形容詞', mastered: false },
  { word: '집', reading: 'jip', japanese: '家', example: '집에 가요.', notes: '名詞', mastered: false },
];

// ===== Storage =====
const STORAGE_KEYS = {
  WORDS: 'korean_words',
  QUIZ_HISTORY: 'korean_quiz_history',
};
...
function getSortedFilteredWords() {
  ...
  else if (sort === 'alpha-asc') list.sort((a, b) => a.word.localeCompare(b.word));
  else if (sort === 'alpha-desc') list.sort((a, b) => b.word.localeCompare(a.word));
  return list;
}
...
function openDeleteModal(id) {
  ...
  if (!w) return;
  pendingDeleteId = id;
  deleteWordName.textContent = `"${w.word}" (${w.japanese})`;
...
wordForm.addEventListener('submit', e => {
  ...
  if (id) {
    ...
    if (w) {
      w.word     = kor;
      w.reading  = formReading.value.trim();
      w.japanese = jpn;
    ...
    words.push({
      id: Date.now(),
      word: kor,
      reading: formReading.value.trim(),
      japanese: jpn,
...
function renderCard() {
  ...
  if (fcReading) {
    ...
    fcReading.style.display = w.reading ? 'block' : 'none';
  }
...
function renderQuizQuestion() {
  ...
  quizWordEl.innerHTML       = `${escHtml(q.word.word)}${q.word.reading ? `<div class="quiz-reading">${escHtml(q.word.reading)}</div>` : ''}`;
  ...
  card.addEventListener('click', () => {
    ...
    importPreviewList.innerHTML = set.words.slice(0, 10).map(w =>
      `<div class="preview-word-row">
        <span class="preview-en">${escHtml(w.word)}<br><span class="preview-reading">${escHtml(w.reading || '')}</span></span>
...
importConfirmBtn.addEventListener('click', () => {
  ...
  if (mode === 'replace') {
    ...
    // 重複チェック（韓国語が同じ単語はスキップ）
    const existingWords = new Set(words.map(w => w.word.toLowerCase()));
    const toAdd = newWords.filter(w => !existingWords.has(w.word.toLowerCase()));
    words.push(...toAdd);
